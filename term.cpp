// TODO(tenshik): split this into modules, proper hierarchy
...
void executeProcesses(const vector<command> &procs, int background) {
  ...
  for (int i = 0; i < procs.size(); i++) {
    ...
    if (procs[i].outredirect != NULL) {
      outfd = open(procs[i].outredirect,
...
void executeMultiWatch(const char user_input[]) {
  ...
  if ((wd = inotify_add_watch(inotify_fd, ".",
                              IN_MODIFY | IN_CREATE | IN_ACCESS)) < 0) {
    perror("add watch: ");
    exit(0);
  }
  ...
  for (auto proc : procs) {
    ...
    if (childpid == 0) {
      ...
      int writefd =
          open(filename.c_str(), O_WRONLY | O_CREAT | O_APPEND | O_TRUNC, 0666);
...
// for a string of length n and an alphabet of size k, time for build is
// O(nlogk) and memory is O(n)
struct state {
  int len, link;
  map<char, int> next;
};
...
public:
  ...
  string returnFullString() { return TEXT; }
};
...
public:
  ...
  const deque<string> &getHistory() { return buff; }

  int getSize() { return static_cast<int>(buff.size()); }
    ...
    do {
      ...
      if (inp == '\t') {
        ...
        if (matches.size() == 1) {
          ...
          int lcp_size =
              min(matches[0].size(), matches[num_matches - 1].size());
          int idx = 0;
          while (idx < lcp_size and
                 matches[0][idx] == matches[num_matches - 1][idx])
            idx++;
          lcp_size = idx;
        ...
        if (esc1 == 91) {
          ...
          case 65:
            if (history_nav) {
              ...
              if (history_cur) {
                for (auto c :
                     history::shellHistoy().getHistory()[history_cur - 1]) {
                  user_input[cnt++] = c;
                  printf("%c", c);
                }
              }
